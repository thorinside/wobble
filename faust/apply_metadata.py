...
def process( text ):
	...
	# replace input names (input0, input1, etc.)
	for i in range(16):
		key = 'input%d' % i
		if key in d:
			old = 'DECLARE_NAME( Input, %d )' % (i + 1)
			new = '"%s"' % d[key]
			text = text.replace( old, new )

	# replace output names (output0, output1, etc.)
	for i in range(16):
		key = 'output%d' % i
		if key in d:
			old = 'DECLARE_NAME( Output, %d )' % (i + 1)
			new = '"%s"' % d[key]
			text = text.replace( old, new )
			# also replace output mode name
			old_mode = 'DECLARE_NAME( Output mode, %d )' % (i + 1)
			new_mode = '"%s mode"' % d[key]
